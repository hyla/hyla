...
class Route {
	...
	public function callback($callback, array $params = NULL)
	{
		if ($params === NULL)
		{
			// Default to array(':route', ':uri',':params')
			$params = array(':route', ':uri', ':params', ':request');
		}

		// Store the rule and params for this rule
		$this->_callbacks[] = array($callback, $params);
	...
	public function matches(Request $request)
	{
		...
		foreach ($this->_callbacks as $array)
		{
			...
			// Callbacks are defined as array($callback, $params)
			list($callback, $cb_params) = $array;

			foreach ($cb_params as $key => $param)
			{
				if (is_string($param) AND array_key_exists($param, $bound_values))
				{
					// Replace with bound value
					$cb_params[$key] = $bound_values[$param];
				}
			}

			$passed = call_user_func_array($callback, $cb_params);
